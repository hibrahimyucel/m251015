...
WHERE STF.DATE_ = CAST (GETDATE() AS DATE ) and STF.TRCODe=8 and UL.Name LIKE 'M3'
group by ITM.name
        ,ul.name`;
...
LEFT JOIN LG_020_ITEMS ITM ON ITM.LOGICALREF = STL.STOCKREF
WHERE STF.DATE_ =CAST (GETDATE() AS DATE ) and ul.name = 'M3' 
ORDER BY STF.DATE_
	,STF.FTIME DESC`;
