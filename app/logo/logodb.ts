...
WHERE 
  STF.DATE_ = CAST (GETDATE() AS DATE) 
  AND STF.TRCODE = 8 
group by 
  ITM.NAME, 
  UL.NAME`;

export const sqlInvoiceDaily = `SELECT 
  STF.LOGICALREF, 
  STF.FICHENO, 
  ...
  RIGHT(
    '0' + CAST(
      (STF.FTIME / 65536 / 256) AS VARCHAR(2)
    ), 
    2
  ) + ':' + RIGHT(
    '0' + CAST(
  ...
  stl.LINEEXP ADDR, 
  stl.OUTPUTIDCODE PLAKA, 
  CASE STF.TRCODE WHEN 1 THEN 'Mal alım irsaliyesi' 
  ...
  WHEN 51 THEN 'Sayım Eksiği Fişi' ELSE 'DİĞER' END AS TIP 
FROM 
  LG_020_01_STFICHE STF 
  LEFT JOIN LG_020_01_STLINE STL ON STF.LOGICALREF = STL.STFICHEREF 
  LEFT JOIN LG_020_UNITSETL UL ON ul.LOGICALREF = STL.uomref 
...
ORDER BY 
  ...
  STF.FTIME DESC
`;
export const sqlInvoiceData = `SELECT 
  STF.LOGICALREF, 
  STF.FICHENO, 
  CONVERT(varchar, STF.DATE_, 23) DATE_, 
  STF.FTIME, 
